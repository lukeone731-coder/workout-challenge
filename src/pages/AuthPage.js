...
export default function AuthPage() {
  ...
  const handle = async () => {
    ...
    let err = null;
    if (mode === "signin") err = await signIn(username, password);
    else if (mode === "signup") err = await signUp(username, password);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", display: "flex", flexDirection: "column", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
        {[["signin","Sign In"],["signup","Sign Up"]].map(([m,label]) => (
          <button key={m} onClick={() => { setMode(m); setError(""); }} style={{
            flex: 1, padding: 10, background: mode === m ? "#f97316" : "#111",
      ...
      {mode !== "admin" && (
        <input value={username} onChange={e => setUsername(e.target.value)} placeholder="Username" style={inp} onKeyDown={e => e.key === "Enter" && handle()} />
      )}
      <input type="password" value={password} onChange={e => setPassword(e.target.value)}
        placeholder={mode === "admin" ? "Admin password" : "Password"} style={inp}
        onKeyDown={e => e.key === "Enter" && handle()} />

      {error && <div style={{ color: "#ef4444", fontFamily: "'DM Sans', sans-serif", fontSize: 13, marginBottom: 10 }}>{error}</div>}

      <button onClick={handle} disabled={loading} style={{
        width: "100%", padding: 14, background: loading ? "#555" : "linear-gradient(135deg,#f97316,#ea580c)",
        border: "none", borderRadius: 10, color: "#fff", fontSize: 20, letterSpacing: 2,
        cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Bebas Neue', sans-serif", marginBottom: 16,
      }}>
        {loading ? "PLEASE WAIT..." : mode === "signin" ? "SIGN IN" : mode === "signup" ? "CREATE ACCOUNT" : "ADMIN LOGIN"}
      </button>
