...
export default function GroupsPage() {
  ...
  const handleJoin = async () => {
    ...
    const err = await joinGroup(code);
    if (err) setError(err);
    ...
    <div style={{ padding: "0 16px" }}>
      ...
      <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 12, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={handleJoin} disabled={loading} style={{ background: loading ? "#555" : "#f97316", border: "none", borderRadius: 10, padding: "0 16px", color: "#fff", cursor: loading ? "not-allowed" : "pointer", fontFamily: "'Bebas Neue', sans-serif", fontSize: 16 }}>
            {loading ? "..." : "JOIN"}
          </button>
        </div>
      ...
      {userGroups.length === 0 && <div style={{ color: "#333", fontFamily: "'DM Sans', sans-serif", textAlign: "center", padding: 40, fontSize: 14 }}>You haven't joined any groups yet</div>}
